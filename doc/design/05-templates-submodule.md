...
├── wiki-spec.md                   # CLI 实现契约 (wiki-spec v0.10.0)
├── fixtures/
│   ├── index.md.txt               # wiki/index.md 字面量 (占位符: 2 个)
│   ├── log.md.txt                 # wiki/log.md 字面量 (占位符: 2 个)
│   ├── memory-index.txt           # MEMORY/MEMORY.md 字面量 (无占位符, 与 canonical/ 一致)
│   ├── tags.md.txt                # wiki/tags.md 字面量 (无占位符, 0.8.0+ 引入)
│   ├── scripts.md.txt             # scripts/SCRIPTS.md 字面量 (无占位符, 0.9.0+ 引入)
...
CLI 只读前 7 份（template + 5 fixtures + 1 spec 文档参考，0.10.0 起）；其余是 SKILL session 内的 LLM 参考资料。

> **无占位符少数派**：5 份 fixture 中**只有** `index.md.txt` / `log.md.txt` 含 `{{TOPIC_NAME}}` / `{{SETUP_DATE}}` 占位符——需要 `_substitute`。其他 3 份 + gitignore.txt 均无占位符，CLI 直接读 + atomic_write（详 [10-spec-alignment §10.3 D3](10-spec-alignment-2026-07.md#d3-scriptsmd-落盘流程2026-07-02-复核后修订)）。
...
# 占位符替身后比对 (3 份有占位符):
cmp -s $TMP/mywiki/wiki/index.md                 my_SKILL/.../canonical/index.md
cmp -s $TMP/mywiki/wiki/log.md                   my_SKILL/.../canonical/log.md
cmp -s $TMP/mywiki/CLAUDE.md                     my_SKILL/.../canonical/<topic-rendered-CLAUDE.md>  # 4 占位符替身后
# 无占位符 fixture 直拷比对 (4 份):
cmp -s $TMP/mywiki/MEMORY/MEMORY.md              my_SKILL/.../fixtures/memory-index.txt         # 0.10.0+ path
cmp -s $TMP/mywiki/wiki/tags.md                  my_SKILL/.../fixtures/tags.md.txt                # 0.8.0+
cmp -s $TMP/mywiki/scripts/SCRIPTS.md            my_SKILL/.../fixtures/scripts.md.txt             # 0.9.0+
...
- **不**承载 CLAUDE.md / index.md / log.md / MEMORY.md / tags.md / SCRIPTS.md / .gitignore——这些一律走 SKILL `references/`（含 wiki + workspace 两仓）
...
CLI 在 `render_and_write` 中做占位符替换（**只对 3 份有占位符 fixture 用**：`claude-md-template.md` / `index.md.txt` / `log.md.txt`），assert 无残留。

### 5.5.1 占位符映射（适用于 3 份有占位符 fixture）
...
### 5.5.2 无占位符 fixture（直拷；spec 0.10.0 + SKILL 仓 2026-07-02 修订后）

| fixture | CLI 落盘位置 | 占位符 |
|---|---|---|
| `fixtures/gitignore.txt` | `<wiki>/.gitignore` | 无 |
| `fixtures/memory-index.txt` | `<wiki>/MEMORY/MEMORY.md`（0.10.0+ 移到 wiki 根） | 无 |
| `fixtures/tags.md.txt` | `<wiki>/wiki/tags.md` | 无 |
| `fixtures/scripts.md.txt` | `<wiki>/scripts/SCRIPTS.md` | 无（**注意**——`scripts.md.txt` 历史上曾有 `{{TOPIC_NAME}}`，SKILL 仓 2026-07-02 修订后改回无占位符；CLI 走字节事实，详 [10-spec-alignment §10.3 D3](10-spec-alignment-2026-07.md#d3-scriptsmd-落盘流程2026-07-02-复核后修订)）|

CLI 直接 `read_text` + `atomic_write`，**不** 走 `_substitute`，**不** 需要 mapping 字段扩展。

### 版本号双轨对齐

- SKILL `metadata.wiki_spec_version`（如 `0.10.0`）+ `metadata.workspace_spec_version`（如 `0.3.0`）
- CLI `llmw.WIKI_SPEC_VERSION` / `WORKSPACE_SPEC_VERSION` 常量（同步 bump）
- 升级时任一组必须保持一致；不一致 = SKILL 仓已升 spec 而 CLI 未跟
...
- 推荐：CLI 仓库根 commit 把 submodule 锁到 SKILL 的某个 tag（如 `wiki-spec-v0.10.0` + `workspace-spec-v0.3.0`）
- 这样 CLI 的每个 release 对应固定的 spec 版本

### 升级流程

1. SKILL 仓 bump `metadata.*_spec_version` + 改 fixtures / template
2. CLI 仓拉新 submodule 后跑附录 A 自检；不一致即同步升级 CLI 渲染逻辑或对齐 fixtures
3. CLI 仓同时 bump `llmw.WIKI_SPEC_VERSION` / `WORKSPACE_SPEC_VERSION` 常量
4. 任一组不一致 → SKILL 仓已升 spec 而 CLI 未跟（详 [10-spec-alignment 附录 B](10-spec-alignment-2026-07.md#附录-bspec-版本对齐校验) 校验命令）
...
- `<repo>/my_SKILL/llm-wiki-management/references/fixtures/*.txt`（spec 0.10.0+ 共 5 份）
- `<repo>/my_SKILL/llm-workspace-management/references/workspace-claude-md-template.md`（workspace CLAUDE.md，spec 0.2.0+ 引入）
- `<repo>/my_SKILL/llm-workspace-management/references/fixtures/memory-index.txt`（workspace MEMORY.md，spec 0.3.0+ 引入）

只要 SKILL 维持：

- 3 份带占位符 fixture 字节级稳定（或与 CLI `WIKI_SPEC_VERSION` 同步 bump）
- CLAUDE.md 模板维持 4 占位符 `{{TOPIC_NAME}}` / `{{SETUP_DATE}}` / `{{WIKI_SPEC_VERSION}}` / `{{CLI_VERSION}}`
- `MEMORY/MEMORY.md` / `wiki/tags.md` / `scripts/SCRIPTS.md` 各自的 fixture 维持无占位符 + 顶部说明文本不变

CLI 与 SKILL 即兼容。SKILL 内部任何运行时纪律（ingest / lint / query / scan）变更不影响 CLI。
