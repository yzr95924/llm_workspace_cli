...
> 注意：`llmw/models/` 子包尚未在 `pyproject.toml` 的 `setuptools.packages` 中列出（仅写了 `llmw` / `llmw.workspace` / `llmw.wiki`）。`llmw model` 命令在开发期可走 `PYTHONPATH=.` 跑通，但 `pip install -e .` 出来的 wheel 不含该子包——这是已知缺陷，wheel 安装需补 `pyproject.toml` 后才能完整跑 `model` 子命令。
...
1. **CLI 不写 wiki 内容**：不写 `raw/` / `wiki/` 下任何文件；只写 `workspace.toml`、`<wiki>/wiki_metadata.toml`、`workspace_models.toml`、以及 `init` 时建的 workspace `.gitignore`。<wiki>/CLAUDE.md 由 `setup_wiki.py` 在 `add` 时创建，CLI 不参与该文件任何写入或编辑。
...
- `doc/design/05-templates-submodule.md` — `templates/` + `my_SKILL` 集成
...
仓库内 `MEMORY/` 目录是项目级"为什么 + 边界"记忆的存放点（**不写个人 memory 目录**）。索引见 `MEMORY/MEMORY.md`；每条规则一份独立 markdown，带 frontmatter。提交进代码仓以便协作方看见 + 跟随代码历史回溯。
